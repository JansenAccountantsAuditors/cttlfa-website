...
(function (w) {
  ...
  function build(mountId, cfg, data) {
    ...
    h += '<div class="snap-sec"><h3 class="snap-h3">League sizes &amp; fixtures by division</h3>';
    h += '<p class="snap-bh">Teams entered, and fixtures played versus still to play, per division. Byes excluded.</p>';
    h += '<div class="snap-cols">';
    streams.forEach(function (s) {
      h += '<div class="snap-col"><div class="snap-ch">' + esc(s.label) + ' <small>' + s.count + ' div · ' + fmt(s.teams) + '</small></div>';
      s.divs.forEach(function (d) {
        h += '<div class="snap-row"><div class="snap-rn"><span>' + esc(d.name) + '</span><small>' + fmt(d.played) + ' played · ' + fmt(d.remaining) + ' to play</small></div><b>' + d.teams + '</b></div>';
      });
      h += '</div>';
    });
    h += '</div></div>';
