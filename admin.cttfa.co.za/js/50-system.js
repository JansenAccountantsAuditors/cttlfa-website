/* CTTLFA admin - System Health module (T05). Depends on core globals: $, sb, esc, dR. */
async function renderSysHealth(){
    var root=$("sysHealthRoot"); if(!root) return;
    ...
    try{
      ...
      root.innerHTML='<div class="card"><h3>System health</h3><p class="hint" style="color:#9A3130">Could not load system health: '+esc(e.message||String(e))+'</p></div>';
      return;
    }
    root.innerHTML=sysHealthHtml(data, season);
    var rb=$("shRefresh"); if(rb) rb.onclick=function(){ renderSysHealth(); };
  }
  function shDT(ts){ if(!ts) return '&mdash;'; try{ return new Date(ts).toLocaleString('en-ZA',{day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'}); }catch(e){ return esc(String(ts)); } }
  function shDate(d){ if(!d) return '&mdash;'; try{ return new Date(d).toLocaleDateString('en-ZA',{day:'2-digit',month:'short',year:'numeric'}); }catch(e){ return esc(String(d)); } }
  ...
  function shPill(word,state){ var c={ok:['#E3F0E7','#1f7a4d'],warn:['#FBF1E2','#9F6621'],bad:['#FBECEB','#9A3130']}[state]||['#EFF4F7','#59666B']; return '<span style="display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:700;padding:3px 10px;border-radius:999px;background:'+c[0]+';color:'+c[1]+'"><span aria-hidden="true" style="width:8px;height:8px;border-radius:50%;background:'+c[1]+'"></span>'+esc(word)+'</span>'; }
  function shTile(title,pill,rows,note){
    var kv=rows.map(function(r){ return '<div style="display:flex;justify-content:space-between;gap:10px;font-size:13px;padding:3px 0;border-bottom:1px solid #EFF1F5"><span style="color:#59666B">'+esc(r[0])+'</span><b style="font-variant-numeric:tabular-nums;text-align:right">'+r[1]+'</b></div>'; }).join('');
    return '<div class="card" style="margin:0"><div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:8px"><h3 style="margin:0;font-size:15px">'+esc(title)+'</h3>'+pill+'</div>'+kv+(note?'<p class="hint" style="margin:8px 0 0">'+note+'</p>':'')+'</div>';
  }
  function sysHealthHtml(data,season){
    ...
    var dbTile=shTile('Club debtor data',shPill(dbState.w,dbState.c),[
        ...
        ['Net debtors',D.net_total!=null?dR(D.net_total):'&mdash;']
    ...
    var fxTile=shTile('League &amp; fixtures feed',shPill(fxState.w,fxState.c),[
        ...
        ['Season',(season&&season.label)?esc(season.label):'&mdash;']
    ...
    var wdTile=shTile('Receipts watchdog',shPill(W.alert_active?'Alert':'Normal',W.alert_active?'warn':'ok'),[
      ...
      ],W.alert_active?('Watchdog alert: '+esc(W.alert_kind||'see Club Debtors')):'No receipts-processing alert.');
    var h='';
